...
context('Gear - Dialog', () => {
  ...
  const successAddMessage = 'Gear added successfully.';
  const successUpdateMessage = 'Gear updated successfully.';
  const successDeleteMessage = 'Gear deleted successfully.';
  ...
  beforeEach(() => {
    ...
    cy.testId('card-gear').within(() => {
      cy.testId('add-gear').click();
    });

    // aliases
    cy.testId('gear-settings-dialog').as('dialog');
  });
  ...
  it('should have title store', () => {
    cy.get('@dialog').within(() => {
      cy.get('h4').should('have.text', 'Gear');
    });
  });

  it('should have no gear', () => {
    cy.get('@dialog').within(() => {
      cy.get('p').should('have.text', 'No Gear yet.');
    });
  });
  ...
  it('should have add button and be enabled', () => {
    cy.get('@dialog').within(() => {
      ...
      cy.testId('add').should('have.text', 'Add Gear');
    });
  });
  ...
  it('should have field labels', () => {
    ...
    cy.testId('form').within(() => {
      cy.testId('gearTypeId-label').should('exist');
      cy.testId('gear-label').should('have.text', 'Search / Add Gear');
      cy.testId('link-label').should('have.text', 'Link');
      cy.testId('information-label').should('have.text', 'Information');
      cy.testId('gearImage').should('exist');
    });
  });
